...
var (
	...
	// Publisher는 최초 Publisher인지, Subscriber인지를 명시하기 위함
	IsPublisher = &cli.BoolFlag{
		Name:  "IsPublisher",
		Usage: "Publisher, Subscriber 구분 인자 (Default: false)",
		Value: false,
	}
	// RedisDBAddrFlag는 Public Key 저장을 위한 Redis DB 주소를 명시
	RedisDBAddrFlag = &cli.StringFlag{
		Name:  "redis-addr",
		Usage: "Public Key를 등록할 Redis DB 주소",
	}
	// PoolSizeFlag는 Redis DB에 연결할 Client의 수를 지정
	PoolSizeFlag = &cli.IntFlag{
		Name:  "pool-size",
		Usage: "Redis DB에 연결할 Client 수 지정",
		Value: 40,
	}
	// MaxIdleConnsFlag는 유지할 연결 수의 최대 한도
	MaxIdleConnsFlag = &cli.IntFlag{
		Name:  "max-idle-conns",
		Usage: "유지할 연결 수의 최대 한도",
		Value: 100,
	}
	// WebPortFlag는 Web Server Port를 지정
	...
	MonitorUDPPortFlag = &cli.UintFlag{
		Name:  "monitor-udp-port",
		Usage: "Monitor UDP Port",
	}
)
